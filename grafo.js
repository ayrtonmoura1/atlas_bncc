...
(() => {
  ...
  function sourceSection(n, r) {
    const source = r.source_url ? `<div class="source-box"><strong>${r.source_kind==='official_bncc'?'Referência oficial':'Referência em revisão'}</strong><p>${esc(sourceText(r.source_note || ''))}</p><a href="${esc(r.source_url)}" target="_blank" rel="noopener noreferrer">Consultar no MEC${r.source_page?` · p. ${r.source_page}`:''}</a>${r.competency_source_url?`<p><a href="${esc(r.competency_source_url)}" target="_blank" rel="noopener noreferrer">Referencial de competências</a></p>`:''}${institutionalSourcesHtml()}${r.candidate_code?`<p>Possível correção não confirmada: ${esc(r.candidate_code)}</p>`:''}</div>` : '';
    ...
    return `${source}<p class="footer-note">Fichas e relações originais: BNCC_Foco.xlsx. Fichas novas: BNCC oficial do MEC (2018). Orientações pedagógicas novas são editoriais do atlas.<br>${institutionalSourcesHtml()}</p><div class="detail-section"><h3>Evidências das relações</h3>${evidence || '<p class="small">Nenhuma relação explícita extraída.</p>'}</div>`;
  }
